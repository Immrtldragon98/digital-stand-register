...
class OperationService:
    ...
    def _get_stand_locked(self, code: str) -> StandAsset:
        stand = (
            self.db.query(StandAsset)
            .filter(StandAsset.code == code)
            .with_for_update()
            .first()
        )
        if not stand:
    ...
    def install_stand(self, stand_code: str, position_id: int, user_id: int | None, operator_name: str | None = None):
        ...
        self.db.add(ActivityLog(
            user_id=user_id,
            action="INSTALL_STAND",
            timestamp=now,
            details=f"Stand {stand.code} installed at Position ID {position_id} by {operator_name or 'not recorded'}.",
        ))
    ...
    def remove_stand(self, stand_code: str, user_id: int | None, operator_name: str | None = None, reason: str | None = None):
        ...
        if stand.current_location != LocationEnum.WRM_LINE or stand.current_status != StatusEnum.INSTALLED:
            raise HTTPException(status_code=400, detail="Stand must be installed on a WRM line before removal.")
        ...
        if not active:
            raise HTTPException(status_code=409, detail="Active installation record is missing. Refresh before continuing.")
        ...
        # Plant rule: a removed running stand always returns to Pending.
        ...
        self.db.add(ActivityLog(
            ...
            action="REMOVE_STAND",
            timestamp=now,
            details=f"Stand {stand.code} removed to PENDING. Reason: {reason or 'not recorded'}. Operator: {operator_name or 'not recorded'}.",
        ))
        self._commit_or_conflict("Stand removal conflicted with another update. Refresh and try again.")
        return {
            "status": "success",
            "message": f"Stand {stand.code} removed and marked PENDING.",
    ...
    def change_stand(self, payload, user_id: int | None):
        ...
        if not active:
            raise HTTPException(status_code=409, detail="Active installation record not found. Refresh and try again.")
        ...
        self.db.add(StandInstallation(
            stand_id=installed.id,
            position_id=payload.position_id,
            installed_at=now,
            installed_by=payload.changed_by,
        ))
        self.db.add(StandChangeEvent(
            position_id=payload.position_id,
            removed_stand_id=removed.id,
            installed_stand_id=installed.id,
            changed_at=now,
            changed_by=payload.changed_by,
            reason=payload.reason,
            notes=payload.notes,
            removed_condition=payload.removed_condition,
        ))
        self.db.add(ActivityLog(
            user_id=user_id,
            action="CHANGE_STAND",
            timestamp=now,
            details=(
                f"Position {payload.position_id}: {removed.code} -> {installed.code}. "
                f"Reason: {payload.reason}. Operator: {payload.changed_by}."
            ),
        ))
        self._commit_or_conflict("Another user changed this stand or position. Refresh and try again.")
        return {
            "status": "success",
            "removed_stand": removed.code,
            "installed_stand": installed.code,
            "removed_campaign_hours": hours,
            "removed_lifetime_hours": removed.lifetime_hours,
            "removed_new_status": StatusEnum.PENDING,
            "installed_new_status": StatusEnum.INSTALLED,
        }
    ...
    def update_preparation_status(
        ...
        if current == StatusEnum.INP:
            if status != StatusEnum.PENDING:
                raise HTTPException(
                    status_code=400,
                    detail="Legacy INP stands must be moved to PENDING before continuing preparation.",
                )
            current_index = -1
            target_index = PREPARATION_FLOW.index(status)
            is_next_step = False
            is_rework_reset = True
        elif current not in PREPARATION_FLOW:
        ...
        if not (is_next_step or is_rework_reset):
            ...
            raise HTTPException(
                status_code=400,
                detail=f"Invalid workflow jump {current.value} -> {status.value}. Next allowed status: {expected}.",
            )

        operator = updated_by.strip()
        if not operator:
            raise HTTPException(status_code=400, detail="Updated by / operator is required.")
        ...
        self.db.add(StandPreparationEvent(
            stand_id=stand.id,
            from_status=current,
            to_status=status,
            updated_by=operator,
        ...
        self.db.add(ActivityLog(
            ...
            timestamp=now,
            details=(
                f"Stand {stand.code} status changed from {current.value} to {status.value} "
                f"by {operator}. Remarks: {(remarks or 'not recorded').strip() or 'not recorded'}."
            ),
        ))
        self._commit_or_conflict("Stand status was changed by another user. Refresh and try again.")
        return {
            "status": "success",
