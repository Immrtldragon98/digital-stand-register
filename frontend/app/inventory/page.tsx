...
type Item = {
  id:number; name:string; quantity:number; minimum_quantity:number; location:string; remarks?:string|null; is_active:boolean;
...
type FormState = {
  name:string; quantity:number; used_at:string; expected_life_hours:string; reliability_pct:string; availability_pct:string; criticality:string; remarks:string;
};

const blank:FormState={name:"",quantity:0,used_at:"",expected_life_hours:"",reliability_pct:"",availability_pct:"",criticality:"MEDIUM",remarks:""};
  ...
  async function addSpare(e:React.FormEvent){
    ...
    try{
      await fetchApi("/inventory/",{method:"POST",body:JSON.stringify({
        name:form.name.trim(),quantity:form.quantity,minimum_quantity:0,location:"Store",remarks:form.remarks||null,
  ...
  return <div className="flex-1 bg-industrial-dark min-h-screen text-slate-100 flex flex-col">
    ...
    <main className="p-4 md:p-5 flex-1 space-y-3">
      ...
      {showAdd&&<form onSubmit={addSpare} className="grid grid-cols-2 md:grid-cols-4 xl:grid-cols-8 gap-2 border border-slate-700 rounded-lg p-3 bg-slate-900/50 text-xs">
        ...
        <input className="input-class col-span-2 md:col-span-3" placeholder="Remarks" value={form.remarks} onChange={e=>setForm({...form,remarks:e.target.value})}/>
      ...
      <div className="overflow-auto border border-slate-700 rounded-lg bg-slate-950/30 max-h-[calc(100vh-210px)]">
        <table className="w-full min-w-[1180px] text-xs border-collapse">
          <thead className="sticky top-0 bg-slate-800 text-slate-200"><tr>
            ...
            <th className="border border-slate-700 px-2 py-2 text-left">Remarks</th><th className="border border-slate-700 px-2 py-2 text-center">Action</th>
          </tr></thead>
