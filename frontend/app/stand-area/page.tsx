...
export default function StandAreaPage(){
  ...
  return <div className="dsr-page"><Header title="Stand Area"/><main className="dsr-main"><div className="full-bleed space-y-4">
    <div className="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between"><div><h1 className="text-xl md:text-2xl font-bold text-white">Stand Preparation</h1><p className="text-sm text-slate-300 mt-1">Pending → Components → Gauging → Hydrotest → Ready</p></div><div className="flex gap-2">{isAdmin(user)&&<button onClick={addStand} className="dsr-btn-primary">+ New Stand</button>}<button onClick={load} className="dsr-btn">Refresh</button></div></div>
    ...
    {loading?<div className="dsr-empty">Loading stand preparation...</div>:<div className="space-y-2">{ORDER.map(status=>{const style=STYLE[status];const count=groups[status].length;const isCollapsed=collapsed[status]??false;return <section key={status} className={`rounded-xl border ${style.row} overflow-hidden`}>
      ...
      {!isCollapsed&&<div className="border-t border-white/5 p-3"><div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 xl:grid-cols-8 2xl:grid-cols-10 gap-2">{groups[status].map((stand:Stand)=><div key={stand.id} className={`rounded-lg border ${style.card} p-2.5 min-w-0 transition-colors`}><button onClick={()=>openStand(stand)} className="w-full text-left"><div className="text-lg font-bold text-white truncate">{stand.code}</div><div className="text-xs text-slate-300 mt-1">Life {Number(stand.lifetime_hours||0).toFixed(0)} h</div></button>{canEdit(user)&&status==="PENDING"&&<button disabled={saving} onClick={()=>advance(stand)} className={`mt-2 w-full min-h-[40px] rounded-md border px-2 py-1.5 text-xs font-medium ${style.button}`}>Components</button>}{canEdit(user)&&status!=="PENDING"&&NEXT[status]&&<button disabled={saving} onClick={()=>advance(stand)} className={`mt-2 w-full min-h-[40px] rounded-md border px-2 py-1.5 text-xs font-medium ${style.button}`}>→ {LABEL[NEXT[status]!]}</button>}</div>)}{count===0&&<div className="col-span-full rounded-lg border border-dashed border-slate-700/60 px-3 py-4 text-center text-sm text-slate-500">No stands in this stage</div>}</div></div>}
    </section>})}</div>}
  </div></main>
