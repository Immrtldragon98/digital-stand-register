...
export default function StandAreaPage(){
  const [stands,setStands]=useState<Stand[]>([]);const [loading,setLoading]=useState(true);const [error,setError]=useState("");const [saving,setSaving]=useState(false);const [selected,setSelected]=useState<any>(null);
  ...
  return <div className="flex-1 bg-industrial-dark min-h-screen text-slate-100 flex flex-col">
    ...
    <main className="p-3 md:p-4 flex-1 space-y-2 overflow-hidden">
      <div className="flex items-center justify-between"><div><h1 className="text-lg font-bold">Stand Preparation</h1><p className="text-xs text-slate-400">Yet to Ready → Pending → Gauging → Hydrotest → Ready</p></div><button onClick={load} className="text-xs px-3 py-1.5 rounded border border-slate-700">Refresh</button></div>
        ...
        {ORDER.map(status=><section key={status} className={`border rounded-xl p-2 ${COLORS[status]}`}>
          <div className="grid gap-1.5 min-h-[72px]" style={{gridTemplateColumns:"repeat(16,minmax(0,1fr))"}}>
            ...
            {groups[status].map((stand:Stand)=><div key={stand.id} className="min-w-0 rounded-lg border border-white/15 bg-slate-950/35 px-1.5 py-1.5 flex flex-col justify-between">
              ...
              {NEXT[status]&&<button disabled={saving} onClick={()=>advance(stand)} className="mt-1 text-[9px] rounded border border-white/20 py-1 truncate hover:bg-white/10">→ {LABEL[NEXT[status]!]}</button>}
            </div>)}
          </div>
        </section>)}
      </div>}
