  ...
  return <div className="bg-industrial-card border border-industrial-border rounded-xl p-4 md:p-5 shadow-xl">
    ...
    {tab==="components"&&<div>
      {!prep?<Empty text="No component preparation was recorded for this stand yet."/>:prep.state==="SKIPPED"?<div className="rounded-lg border border-amber-800/60 bg-amber-950/20 p-3 text-sm text-amber-200">Components were skipped before gauging for this preparation cycle.</div>:<>
        ...
        <div className="overflow-x-auto rounded-lg border border-slate-800"><table className="w-full text-xs"><thead className="bg-slate-900 text-slate-400"><tr><th className="p-2 text-left">Component</th><th>New</th><th>Reused</th><th>Running</th><th>Total Life</th><th>Expected</th><th>Criticality</th></tr></thead><tbody>{prep.items?.map((x:any,i:number)=><tr key={i} className="border-t border-slate-800"><td className="p-2 font-semibold text-white">{x.name}</td><td className="text-center">{x.new_qty}</td><td className="text-center">{x.reused_qty}</td><td className="text-center">{fmtHours(x.current_campaign_hours)}</td><td className="text-center">{fmtHours(x.estimated_total_life_hours)}</td><td className="text-center">{x.expected_life_hours?fmtHours(x.expected_life_hours):"—"}</td><td className="text-center">{x.criticality||"—"}</td></tr>)}</tbody></table></div>
        <p className="mt-2 text-[10px] text-slate-500">Reused-component total life becomes accurate when carried life is recorded. Expected life and criticality remain blank until engineering values are configured.</p>
      </>}
    </div>}
