    ...
    <div className="grid gap-4">
      {lines.map(line=>{const tone=lineTone[line.name]||lineTone.W1;return <section key={line.id} className="mechanical-panel overflow-hidden">
        <div className={`flex items-center justify-between px-4 py-3 border-b border-[#26354a] ${tone.header}`}>
          <div className="flex items-center gap-2"><span className={`h-8 w-1 rounded-full ${tone.bar}`}/><div><div className={`text-base font-bold ${tone.badge}`}>{line.name}</div><div className="text-xs text-slate-400">Running line</div></div></div>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-5 md:grid-cols-10 gap-px bg-[#243247] p-px">
          {line.positions?.map((p:any)=>{const s=p.current_stand;const life=s?.life_percent;return <button key={p.id} disabled={!s} onClick={()=>open(s)} className="min-w-0 text-left p-3 bg-[#111827] hover:bg-[#18243a] disabled:opacity-40 transition-colors">
            <div className="flex justify-between items-center gap-2"><span className="text-xs font-bold text-slate-400">P{p.position_number}</span>{life!=null&&<span className={`h-2 w-2 rounded-full ${life>=90?"bg-rose-400":life>=75?"bg-amber-400":"bg-emerald-400"}`}/>}</div>
            ...
            <div className="mt-2 space-y-1 text-xs text-slate-400">
              ...
              {life!=null&&<div className={`font-semibold ${life>=90?"text-rose-300":life>=75?"text-amber-300":"text-emerald-300"}`}><span className="text-slate-500 font-normal">Life </span>{life}%</div>}
            </div>
          </button>})}
        </div>
      </section>})}
