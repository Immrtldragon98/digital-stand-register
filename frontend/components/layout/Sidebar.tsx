...
export default function Sidebar(){
 ...
 const primary=[
  ...
  {name:"PM Activities",href:"/pm-schedule",icon:CalendarDays},
 ...
 const item=(i:any)=>{const I=i.icon;const isActive=active(i.href);return <Link key={i.href} href={i.href} className={`nav-item ${isActive?"nav-item-active":""}`}><I className={`w-4 h-4 shrink-0 ${isActive?"text-[#4EA1FF]":"text-slate-500"}`}/><span>{i.name}</span></Link>};
